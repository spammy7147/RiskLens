...
기본 웹 화면과 사용자 요청 처리는 Spring 기반으로 구성한다.

리스크 분석 모델링, 통계 기반 계산, 점수 산출은 Python 서버에서 담당한다.

Spring 서버는 외부 공공 API 또는 수집 데이터를 저장, 정제, 관리하고 Python 모델링 서버에 필요한 입력 형태로 전달한다. Python 서버는 분석 결과를 계산한 뒤 Spring 서버로 반환하고, Spring 서버는 결과를 화면에 보여준다.

## 2. Responsibility Split

| 영역 | 담당 시스템 | 책임 |
| --- | --- | --- |
| 웹 화면 | Spring + View | 사용자 입력, 결과 조회, 분석 이력 화면 |
| 사용자 요청 처리 | Spring Backend | 요청 검증, 인증/세션, 분석 요청 생성 |
| 데이터 저장 | Spring Backend | 사용자 입력, 분석 결과, 공공 기준 데이터 저장 |
| 외부 API 연동 | Spring Backend | KOSIS, 공공데이터, 보건의료 데이터 수집 |
| 데이터 정제 | Spring Backend | 외부 데이터를 내부 기준 데이터셋 형태로 정규화 |
| 모델 입력 생성 | Spring Backend | Python 서버에 보낼 분석 요청 DTO 구성 |
| 리스크 계산 | Python Modeling Server | 시나리오 계산, 점수 모델, 통계 기반 분석 |
| 결과 해석 데이터 생성 | Python Modeling Server | 부족금액, 대응률, 생존개월, 회복기간, 점수 |
| 결과 저장 | Spring Backend | Python 분석 결과 저장 |
| 결과 표시 | Spring + View | 사용자 리포트 화면 렌더링 |
...
Spring은 웹 애플리케이션, 데이터 저장, 외부 API 연동, 사용자 요청 처리에 적합하다.
...
- Spring이 원천 데이터를 저장한다.
- Spring이 프로젝트 내부 기준 데이터셋 형태로 정제한다.
...
- 사용자는 Spring 화면에서 정보를 입력한다.
- Spring은 입력값을 검증하고 저장한다.
- Spring은 기준 데이터와 사용자 입력을 조합해 Python 서버 요청을 만든다.
- Python 서버는 리스크 계산과 점수화를 수행한다.
- Spring은 결과를 저장하고 화면에 표시한다.

## 5. Boundary Between Spring and Python

두 서버의 경계를 명확히 한다.

### Spring이 판단하지 않는 것

Spring은 최종 리스크 점수나 모델 계산을 직접 수행하지 않는다.
...
Spring과 Python은 HTTP API로 통신한다.
...
정확한 DTO는 `CALCULATION_SPEC.md`와 `BACKEND_DESIGN.md` 작성 시 확정한다.

## 7. Storage Direction

Spring 서버가 저장 책임을 갖는다.
...
- Spring은 분석 상태를 REQUESTED, PROCESSING, COMPLETED, FAILED로 관리한다.
- 실패 사유를 저장한다.
- 재시도 가능 구조를 고려한다.
...
| 문서 | 목적 |
| --- | --- |
| DATASET_SPEC.md | Spring이 저장/정제할 기준 데이터셋 정의 |
| CALCULATION_SPEC.md | Python이 수행할 계산식 정의 |
| API_CONTRACT.md | Spring-Python 요청/응답 DTO 정의 |
| BACKEND_DESIGN.md | Spring 모듈, DB, 분석 이력 구조 정의 |
